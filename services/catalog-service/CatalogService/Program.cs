...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("service", "catalog-service")
    .WriteTo.Console()
    .CreateLogger();
...
builder.Services.AddDbContext<CatalogDbContext>(options =>
{
    if (builder.Environment.IsEnvironment("Testing"))
        options.UseInMemoryDatabase("CatalogServiceTests");
    else
        options.UseNpgsql(
...
app.MapGet("/health", () => Results.Ok(new { status = "ok", service = "catalog-service" }));
...
static async Task InitializeDatabaseAsync(IServiceProvider services)
{
    ...
    for (var attempt = 1; attempt <= 15; attempt++)
    {
        try
        {
            ...
            Console.WriteLine("catalog-service: database ready and seeded.");
            return;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"catalog-service DB init attempt {attempt}/15 failed: {ex.Message}");
            if (attempt == 15) throw;
